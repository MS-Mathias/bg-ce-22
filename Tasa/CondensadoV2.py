...
def ImportaArchivos (años_caidas=10):
    ...
    for archivo in tqdm(Archivos):   # itero por cada nombre de archivo de la lista
        ...
        df3.loc[(df3['Flag CER']==1) &(df3['Moneda']=='ARS') ,'Moneda']='CER'
        ...
        for i in df3.index:
            ...
            if df3.at[i,'Flag_variable']==1:   # detecta si la cuenta es variable y le aplico los cambios correspondientes:
                ...
                suma_caidas_capital = caidas_capital.iloc[fila,columna:].sum()   #  calculo la sumatoria de la caida de la cuenta
                caidas_capital.at[i,30] = suma_caidas_capital   # le inserto esa suma al primer bucket 
                caidas_capital.iloc[fila,(columna+1):] = 0   # transformo el resto de los buckets en "0" para no duplicar el efecto
                
                suma_caidas_capital_e_intereses = caidas_capital_e_intereses.iloc[fila,columna:].sum()
                caidas_capital_e_intereses.at[i,30]=suma_caidas_capital_e_intereses
                caidas_capital_e_intereses.iloc[fila,(columna+1):] = 0
                
                suma_intereses = caidas_intereses.iloc[fila,columna:].sum()
                caidas_intereses.at[i,30]=suma_intereses 
...
Comienza proceso para la tasa {LugarBalance} {Moneda}""")
            ...
            ultimaCurva = pd.DataFrame(tasasInput.loc[(tasasInput['Lugar del balance'] == LugarBalance) &
                                                      (tasasInput["Moneda"] == Moneda),
...
cols=[]
for col in Caidas.columns:
    if isinstance(col,int):
        cols.append(col)

prueba = pd.melt(Caidas_capital, id_vars =['Cuenta','Lugar del balance','Moneda','Flag_variable','Capitalizacion_tasa']
                                , value_vars =cols,var_name='Bucket' ,value_name='Capital') 

prueba2 = pd.melt(Caidas_intereses, id_vars =['Cuenta','Lugar del balance','Moneda','Flag_variable','Capitalizacion_tasa']
                                , value_vars =cols,var_name='Bucket' ,value_name='Interes')            
            
CERT_Caidas = prueba.merge(prueba2,how='inner')
CERT_Caidas['Monto'] = CERT_Caidas['Capital'] + CERT_Caidas['Interes']              
CERT_CaidasV2=pd.melt(Caidas, id_vars =['Cuenta','Lugar del balance','Moneda','Flag_variable','Capitalizacion_tasa']
                                , value_vars =cols,var_name='Bucket' ,value_name='Monto')
