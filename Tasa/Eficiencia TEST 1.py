...
def simulacionCurva(dfDiferencia, ultimaCurva, cholesky, M=1000, arrayShockIndependiente="PRIMERO"):
    ...
    for nodo in dfDiferencia: ### ciclo todos los nodos
        avg[nodo] = dfDiferencia.mean() ### calculo el promedio del nodo
        std[nodo] = dfDiferencia.std() ### calculo el desvio del nodo
    ...
    if type(arrayShockIndependiente) == str:
        ...
        for nodo in dfRandom: ### ciclo todos los nodos
            dfRandom[nodo] = ((dfDiferencia[nodo].quantile(dfRandom[nodo]) - avg[nodo]) / std[nodo]).values ### tomo el percentil y lo estandarizo

        arrayShockIndependiente = dfRandom.values ### Guardo el array de shocks independientes
        
        for i in range(M): ### ciclo a travez de todas las simulaciones
...
for nodo in dfDiferencia:
    avg[nodo] = dfDiferencia[nodo].mean()
    std[nodo] = dfDiferencia[nodo].std()
M = 100
arraySimulaciones = np.zeros(shape=(M, len(ultimaCurva)))
arrayRandom = np.random.rand(M,19)
dfRandom = pd.DataFrame(arrayRandom,columns = nodosAsist)

dfRandom.sum()

for nodo in dfRandom:
    dfRandom[nodo] = ((dfDiferencia[nodo].quantile(dfRandom[nodo]) - avg[nodo]) / std[nodo]).values
